    ...
    if request.method == 'POST':
        ...
        if filename != '':
            ...
            if file_ext not in app.config['ALLOWED_EXTENSIONS'] or file_ext != validate_file(uploaded_file.stream):
                flash(message='Wrong format or damaged file', category='danger')
            else:
                uploaded_file.save(
...
def result():
    ...
    if isinstance(output, str): #check if output of func is str
        return render_template('result.html', title='Results', output=output)
        ...
        return render_template('result.html', title='Results', columns1=dataframe_1_columns, value1=dataframe_1_values, columns2=dataframe_2_columns, value2=dataframe_2_values)
# ROUTES #
